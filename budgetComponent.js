...
import { getPersonalRemainingBudget,addBudgetCounter,updateBudgetCounter,resetPersonalBudget} from './database'


function BudgetCounter({ budgetCounter,setBudgetCounter}) { 
  ...
  useEffect(() => {
    const fetchBudget = async () => {
      const budget = await getPersonalRemainingBudget();
...
const handleBudgetCounterInput = (input) => { 
  ...
  const handleBudgetCounter = async () => {
    if (budgetCounter == null) {
      await addBudgetCounter(budgetValue,budgetValue);
    } else { 
      await updateBudgetCounter(budgetValue);
    }
